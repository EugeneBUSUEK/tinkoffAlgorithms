...
счетчик.
### Task25
Заводим set для того, чтобы копить уникальные элементы. Проходимся по массиву с конца. На каждой итерации добавляем 
элемент массива в set затем на переписываем значения элемента на количество элементов в set, так мы знаем сколько 
уникальных цифр храниться в подмассиве после ai-ого элемента. Далее просто обращаемся к нашему переписанному массиву
по индексу l-1.
### Task26
...
### Task29
Идем по массиву и смотрим: Если встретили четное число то берем его как подмножество из одного элемента. Если нашли 
нечетное число в первый раз запоминаем его индекс и ищем дальше либо четное, либо второе нечетное. Нашли второе нечетное 
выводим количество и все индексы с сохраненного до текущего. На каждой итерации не забываем про счетчик.
### Task30
Никакого ДП и даже не жадный алгос. Начинаме итерироваться и каждый раз прибавляем 1 к следующей горсти конфет 
для ребенка. На каждой итерации проверяем что если прибавив к общей сумме уже розданных конфет полученное число у нас 
еще остается раздать конфет строго больше чем то число которое мы прибавили. Так крутимся в цикле пока не нарушим 
вышеописанное условие. Тогда считаем сколько конфет нам еще осталось раздать. И выбираем это значение как горсть конфет 
для последнего ребенка.
### Task31
Структура предоставляемая на вход по факту просто деревья. Ответом будет просто максимальная глубина среди всех деревьев.
Создаем функцию, которая принимает на вход индекс сотрудника. Затем просто идем вверх до его самого верхнего начальника, 
тот у которого -1. Попутно считаем сколько итераций нам пришлось провести. Делая так по каждому сотруднику в массиве 
сравниваем полученные значения и выбираем самое большое.
### Task32
Рекурсивно спускаемся по воображаемому графу. Рекурсивная функция будет принимать индекс ячейки от которой мы 
перемещаемся. Если ячейка в которую мы планируем переместиться равна индексу (в нашем случаее номаеру) искомой ячейки 
вернуть true обратно по всему дереву вызовов функции. Если после перемещения мы попадаем в последнюю ячейку, то 
возвращать false по всему дереву вызовов. Если ни одоно из вышеперечисленных условий не выполнилось вызывать снова 
рекурсивную функцию с параметром start равным i + ai (в нашем случае i - это start).
### Task34
Возьмем за корень нашего графа первый элемент(район). Далее в цикле пройдемся по всем остальным элементам и связываем с 
первым элементом все, которые от него отличаются (то есть не принадлежат одной банде). Попутно запомним индекс любого 
из привязанных элементов. Затем снова пройдем циклом по всем элементам на этот раз привязывать будем элементы, которые 
равны первому (то есть принадлежат той же банде, что и наш корень) к тому элементу, индекс которого мы запомнили ранее. 
Таким образом мы точно уверены, что все районы связаны и эта связь удовлетворяет условию задачи. В начале когда 
читаем данные из консоли проверяем что количество банд строго больше 1 иначе сразу выводим "NO".
### Task35
Обычный алгоритм обхода в глубину через рекурсию. Обход in-order то есть сначала рекрсивно заходим в левое поддерево 
затем обрабатываем середину (в нашем случае выводим deep) и потом заходим в правое поддерево. Рекурсивная функция 
находит максимальный элемент в переданном подмассиве, после чего вызывает саму себя для левого подмассива от 
максимального элемента переданного подмассива, затем выводит глубину данной обрабатываемой вершины, только потом 
вызывает саму себя для правого подмассива от максимального элемента переданного подмассива. Если функция дошла до 
подмассива из 1 элемента, то просто выводим глубину обрабатываемой вершины. Если же подмассив пустой 
(indexFrom == indexTo), то выходим из функции.